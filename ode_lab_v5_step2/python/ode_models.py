...
# -------- Standardized "Series" output (matches WASM UI shape) --------
def to_series(t: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
    """
    Normalize solver output to a JS/WASM-friendly shape:
      { "t": [..], "y": [..flattened..], "dim": N }
    where y is flattened row-major over time (t index first).
    """
    y2 = np.asarray(y)
    if y2.ndim == 1:
        y2 = y2.reshape(-1, 1)
    return {"t": np.asarray(t).tolist(), "y": y2.reshape(-1).tolist(), "dim": int(y2.shape[1])}

def integrate_series(model: str, y0: List[float], t0: float, t1: float, dt: float, params: Dict[str, Any], backend: str = "auto") -> Dict[str, Any]:
    t, y = integrate_model(model=model, y0=y0, t0=t0, t1=t1, dt=dt, params=params, backend=backend)
    return to_series(t, y)
