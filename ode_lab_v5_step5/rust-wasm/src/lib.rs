...
pub struct SolverStats {
    ...
    #[serde(rename = "steps")]
    pub steps: usize,
    #[serde(rename = "newton_iters")]
    pub newton_iters: usize,
    #[serde(rename = "newton_fail")]
    pub newton_fail: usize,
}
...
fn integrate_rk4_vec(
    ...
    Series { t: ts, y: ys, dim: n, stats: None }
}
...
fn integrate_implicit_euler_vec(
    rhs: RhsFn,
    params: &std::collections::BTreeMap<String, f64>,
    mut t: f64,
    t1: f64,
    ...
    let mut newton_iters_total = 0usize;
    let mut newton_fail = 0usize;
    let method = "implicit_euler".to_string();
    ...
    for _ in 0..max_steps {
        ...
        rhs(t, &y, &mut f0, params);
        for i in 0..n { y_next[i] = y[i] + h*f0[i]; }

        let mut ok = false;
        for _it in 0..newton_max {
            newton_iters_total += 1;
            // g = y_next - y - h f(tn1, y_next)
            rhs(tn1, &y_next, &mut f1, params);
            for i in 0..n {
                g[i] = y_next[i] - y[i] - h*f1[i];
            }
            ...
            // J = I - h df/dy, compute numeric df/dy by forward differences
            // base f1 already computed at y_next
            for col in 0..n {
                let mut ypert = y_next.clone();
                let eps = jac_eps * (1.0 + y_next[col].abs());
                ypert[col] += eps;
                let mut fpert = vec![0.0; n];
                rhs(tn1, &ypert, &mut fpert, params);
                for row in 0..n {
                    let dfdx = (fpert[row] - f1[row]) / eps;
                    jac[row*n + col] = -h * dfdx; // start with -h*df/dy
                }
            }
            // add I
            for i in 0..n { jac[i*n + i] += 1.0; }

            // Solve J * delta = -g
            let rhs_b = g.iter().map(|v| -v).collect::<Vec<f64>>();
            if let Some(sol) = solve_linear(jac.clone(), rhs_b, n) {
                delta.copy_from_slice(&sol);
            } else {
                break;
            }

            // update
            for i in 0..n { y_next[i] += delta[i]; }
            let norm_d = delta.iter().map(|v| v*v).sum::<f64>().sqrt();
            if norm_d < newton_tol {
                ok = true;
                break;
            }
        }
        ...
        if !ok {
            newton_fail += 1;
            // fallback: take explicit Euler step (keeps sim moving, but flags stats)
            for i in 0..n { y_next[i] = y[i] + h*f0[i]; }
        }
    ...
    Series {
        ...
        stats: Some(SolverStats{
            method,
            steps: ts.len().saturating_sub(1),
            newton_iters: newton_iters_total,
            newton_fail,
        }),
    }
}
...
#[wasm_bindgen]
pub fn integrate_model_implicit(model: String, params: JsValue, y0: Vec<f64>, t0: f64, t1: f64, dt: f64, newton_tol: f64, newton_max: usize, jac_eps: f64) -> JsValue {
    let p = params_from_js(params);
    let series = match model.as_str() {
        "linear" => integrate_implicit_euler_vec(rhs_linear_2x2, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
        "vdp" => integrate_implicit_euler_vec(rhs_vanderpol, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
        "lv" => integrate_implicit_euler_vec(rhs_predator_prey, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
        "duffing" => integrate_implicit_euler_vec(rhs_duffing, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
        "sir" => integrate_implicit_euler_vec(rhs_sir, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
        "lorenz" => integrate_implicit_euler_vec(rhs_lorenz63, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
        "rossler" => integrate_implicit_euler_vec(rhs_rossler, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
        "chua" => integrate_implicit_euler_vec(rhs_chua, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
        "thomas" => integrate_implicit_euler_vec(rhs_thomas, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
        _ => integrate_implicit_euler_vec(rhs_linear_2x2, &p, t0, t1, y0, dt, newton_tol, newton_max, jac_eps),
    };
