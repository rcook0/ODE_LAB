...
const MODEL_REGISTRY = [
  {
    ...
    defaultProj:[0,1],
    params:[
      {k:'a11', label:'a11', def:0.0, step:0.1},
      {k:'a12', label:'a12', def:1.0, step:0.1},
      {k:'a21', label:'a21', def:-1.0, step:0.1},
      {k:'a22', label:'a22', def:0.0, step:0.1},
    ],
    field2d:(x,y,p)=>[p.a11*x + p.a12*y, p.a21*x + p.a22*y],
  },
  {
    key:'vdp',
    label:'Van der Pol',
    ...
    defaultY0:[2,0],
    defaultProj:[0,1],
    params:[{k:'mu', label:'μ', def:1.0, step:0.1}],
    field2d:(x,y,p)=>[y, p.mu*(1-x*x)*y - x],
  },
  {
    key:'lv',
    label:'Predator–Prey',
    ...
    defaultProj:[0,1],
    params:[
      {k:'alpha',label:'α',def:1.0,step:0.05},
      {k:'beta', label:'β',def:0.1,step:0.01},
      {k:'delta',label:'δ',def:0.075,step:0.01},
      {k:'gamma',label:'γ',def:1.5,step:0.05},
    ],
    field2d:(x,y,p)=>[p.alpha*x - p.beta*x*y, p.delta*x*y - p.gamma*y],
  },
  {
    key:'duffing',
    ...
    defaultProj:[0,1],
    params:[
      {k:'delta',label:'δ',def:0.2,step:0.02},
      {k:'alpha',label:'α',def:-1.0,step:0.1},
      {k:'beta', label:'β',def:1.0,step:0.1},
      {k:'gamma',label:'γ',def:0.3,step:0.05},
      {k:'omega',label:'ω',def:1.2,step:0.05},
    ],
    field2d:(x,y,p)=>[y, -p.delta*y - p.alpha*x - p.beta*x*x*x + p.gamma*Math.cos(p.omega*0)], // local linearized at t=0 for field
  },
  {
    key:'sir',
    label:'SIR',
    dim:3,
    defaultY0:[0.99,0.01,0.0],
    defaultProj:[0,1],
    params:[
      {k:'beta', label:'β', def:0.5, step:0.02},
      {k:'gamma',label:'γ', def:0.2, step:0.02},
    ],
    // 2D phase-plane uses S vs I with R eliminated or held implicit; show dS/dt, dI/dt with R=1-S-I
    field2d:(s,i,p)=>{
      const r = Math.max(0, 1 - s - i);
      return [-p.beta*s*i, p.beta*s*i - p.gamma*i];
    },
  },
  {
    key:'lorenz',
    ...
    defaultProj:[0,1],
    params:[
      {k:'sigma',label:'σ',def:10.0,step:0.5},
      {k:'rho',  label:'ρ',def:28.0,step:0.5},
      {k:'beta', label:'β',def:8/3,step:0.1},
      {k:'z0',   label:'z0 (slice)',def:1.0,step:0.1},
    ],
    field2d:(x,y,p)=>[p.sigma*(y-x), x*(p.rho - p.z0) - y],
  },
  {
    key:'rossler',
    label:'Rössler (3D)',
    ...
    defaultProj:[0,1],
    params:[
      {k:'a',label:'a',def:0.2,step:0.01},
      {k:'b',label:'b',def:0.2,step:0.01},
      {k:'c',label:'c',def:5.7,step:0.1},
      {k:'z0',label:'z0 (slice)',def:0.1,step:0.1},
    ],
    field2d:(x,y,p)=>[-y - p.z0, x + p.a*y],
  },
  {
    key:'chua',
    label:'Chua (3D)',
    ...
    defaultProj:[0,1],
    params:[
      {k:'alpha',label:'α',def:15.6,step:0.2},
      {k:'beta', label:'β',def:28.0,step:0.5},
      {k:'m0',   label:'m0',def:-1.143,step:0.01},
      {k:'m1',   label:'m1',def:-0.714,step:0.01},
      {k:'z0',   label:'z0 (slice)',def:0.0,step:0.1},
    ],
    field2d:(x,y,p)=>{
  ...
  },
  {
    ...
    defaultProj:[0,1],
    params:[
      {k:'a',label:'a',def:0.208186,step:0.001},
      {k:'z0',label:'z0 (slice)',def:0.0,step:0.1},
    ],
    field2d:(x,y,p)=>[Math.sin(y) - p.a*x, Math.sin(p.z0) - p.a*y],
  },
];

function getModel(key){ return MODEL_REGISTRY.find(m=>m.key===key) || MODEL_REGISTRY[0]; }

// ---- Projection pipeline (Step 3) ----
function projectSeries(series, i, j){
  const {t, y, dim} = series;
  const n = t.length;
  ...
  return { t, y: out, dim: 2, proj:[i,j] };
}

function defaultProjection(dim){
  if(dim >= 2) return [0,1];
  return [0,0];
}

import init, {integrate_linear_2x2, integrate_vanderpol, integrate_predator_prey,
...
, integrate_model, integrate_model_implicit } from './pkg/ode_wasm.js';
...
function mountParams(modelKey){
  const m = getModel(modelKey);
  const card = el('params');
  // Build form rows from metadata
  const rows = [];
  for(const prm of m.params){
    const id = `p_${prm.k}`;
    rows.push(`<div class="row"><label class="mono">${prm.label}</label><input id="${id}" type="number" step="${prm.step ?? 0.1}" value="${prm.def}"></div>`);
  }
  card.innerHTML = `<div class="mono small">${m.label}</div>${rows.join('')}`;

  // Projection dropdowns based on dim
  const pi = el('projI'), pj = el('projJ'), pe = el('projEnable');
  pi.innerHTML=''; pj.innerHTML='';
  for(let k=0;k<m.dim;k++){
    const oi=document.createElement('option'); oi.value=String(k); oi.textContent=`x${k}`;
    const oj=document.createElement('option'); oj.value=String(k); oj.textContent=`x${k}`;
    pi.appendChild(oi); pj.appendChild(oj);
  }
  const defProj = m.defaultProj || defaultProjection(m.dim);
  projI = defProj[0]; projJ = defProj[1];
  pi.value=String(projI); pj.value=String(projJ);

  // Enable projection automatically for dim>2 (you can turn it off)
  projEnabled = (m.dim > 2);
  pe.checked = projEnabled;
}
...
function currentParams(){
  ...
  const p = {model};
  for(const prm of m.params){
    const v = parseFloat(el(`p_${prm.k}`).value);
    p[prm.k] = isFinite(v) ? v : prm.def;
  }
  // common controls (if present)
  p.dt = parseFloat(el('dt')?.value ?? '0.01');
  p.T  = parseFloat(el('T')?.value ?? '50');
  return p;
}
...
function integrateAndDraw(opts={forceIntegrate:true}){
  ...
  if(['linear','vdp','lv','duffing','sir'].includes(p.model)){
    ...
    for(const s of seeds){
      ...
      try{
        let res=null;
        if(p.model==='linear') res = normalizeSeries(integrate_linear_2x2(p.a11,p.a12,p.a21,p.a22,s.x,s.y,0,p.T,p.dt));
        if(p.model==='vdp') res = normalizeSeries(integrate_vanderpol(p.mu,s.x,s.y,0,p.T,p.dt));
        if(p.model==='lv') res = normalizeSeries(integrate_predator_prey(p.alpha,p.beta,p.delta,p.gamma,s.x,s.y,0,p.T,p.dt));
        if(p.model==='duffing') res = normalizeSeries(integrate_duffing(p.delta,p.alpha,p.beta,p.gamma,p.omega,s.x,s.y,0,p.T,p.dt));
        if(p.model==='sir') res=integrate_sir(p.beta,p.gamma,s.x,s.y,Math.max(0,1-s.x-s.y),0,p.T,p.dt);
        if(res){ trajCache.set(k, res); drawTrajectory((projEnabled && res.dim>2) ? projectSeries(res, projI, projJ) : res); }
  ...
  try{
    ...
    if(p.model==='chua') res = normalizeSeries(integrate_chua(p.alpha,p.beta,p.m0,p.m1,x0,y0,z0,0,p.T,p.dt));
    if(p.model==='thomas') res = normalizeSeries(integrate_thomas(p.a,x0,y0,z0,0,p.T,p.dt));
    const pts=[];
    for(let i=0;i<res.t.length;i++){
...
solverSel.onchange=()=>integrateAndDraw({forceIntegrate:true});
newtonTolEl.oninput=()=>integrateAndDraw({forceIntegrate:true});
newtonMaxEl.oninput=()=>integrateAndDraw({forceIntegrate:true});
jacEpsEl.oninput=()=>integrateAndDraw({forceIntegrate:true});
...
modelSel.onchange=()=>{
  ...
  // reset seeds to model default (2D uses first two components)
  seeds = [{x:m.defaultY0[0] ?? 1, y:m.defaultY0[1] ?? 0}];
  if(!modelSel.value) modelSel.value = MODEL_REGISTRY[0].key;
  mountParams(modelSel.value);
  integrateAndDraw({forceIntegrate:true});
};
...
window.addEventListener('keydown', (e)=>{
  if(e.key === 'p'){ projEnabled = !projEnabled; redraw(); }
  if(e.key === '['){ projI = Math.max(0, projI-1); redraw(); }
  if(e.key === ']'){ projI = projI+1; redraw(); }
  if(e.key === '{'){ projJ = Math.max(0, projJ-1); redraw(); }
  if(e.key === '}'){ projJ = projJ+1; redraw(); }
});
...
function renderStats(res){
  ...
  const s = res.stats;
  const card = el('params');
  const line = `<div class="mono small" style="margin-top:8px;opacity:.8;">solver:${s.method} steps:${s.steps} newton_iters:${s.newton_iters} fails:${s.newton_fail}</div>`;
  // append once per run
  if(!card.innerHTML.includes("solver:")) card.innerHTML += line;
}
